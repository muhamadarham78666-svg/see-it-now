      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="glass-card p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 items-start">
            {/* Left: video */}
            <div className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 h-[340px] sm:h-[380px] lg:h-[420px]">
              <GuideAnimation />
            </div>

            {/* Right: what you'll learn */}
            <ol className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-x-6 gap-y-3 lg:gap-y-4">
              {guideSteps.map((step, i) => (
                <li key={step} className="flex items-center gap-2.5 lg:gap-3 min-w-0">
                  <span className="w-6 h-6 lg:w-7 lg:h-7 flex-shrink-0 rounded-full bg-gradient-to-br from-primary-500 to-accent-500 text-white text-xs font-bold flex items-center justify-center">
                    {i + 1}
                  </span>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white leading-snug">{step}</p>
                </li>
              ))}
            </ol>
          </div>
