              ...
              <Card key={request.id} className="p-5 space-y-4">
                ...
                {isOpen && (
                  <div className="border-t border-slate-200 dark:border-slate-700 pt-4 space-y-3">
                    ...
                    <div className="flex gap-2 flex-wrap"><input value={subscriptionForm.password} onChange={(event) => setSubscriptionForm({ ...subscriptionForm, password: event.target.value })} placeholder="New account password" className="input-field flex-1 min-w-[220px]" /><button type="button" onClick={() => setSubscriptionForm({ ...subscriptionForm, password: randomPassword() })} className="px-3 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300" title="Generate password"><RefreshCw size={15} /></button><button disabled={busy === `sa-${request.id}`} onClick={() => void act(`sa-${request.id}`, async () => { const result = await activateSubscription({ data: { token: tk, requestId: request.id, password: subscriptionForm.password || null, startsAt: null } }); if (result.ok) setSubscriptionForm(null); return result; })} className="btn-primary text-sm">{busy === `sa-${request.id}` ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />} Create/link & activate</button></div>
                  </div>
                )}
              </Card>
            );
